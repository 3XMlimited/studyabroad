...
const allQuestions = ['Do you want to study in a country where English is the primary language spoken ?',
...
const Questions = () => {
  ...
  const completionPercentage = ((answeredCount / totalQuestions) * 100).toFixed(2);
  ...
  return (
    <div className="bg-black min-h-screen h-full w-full min-w-full">
      <div className="mx-2 lg:mx-auto justify-center align-middle my-auto">
        <div className={`lg:w-[50vw] flex flex-col justify-center mx-auto py-auto h-full align-middle content-center`}>
          <Image height={'4rem'} width={'4rem'} src={a} alt="Hat" />
          {
            answeredCount>0 && (
              <button onClick={goBack} className={`text-white ${jost.className}  relative lg:right-96 right-40 md:right-96`}><div className='flex justify-center content-center align-middle'>< IoMdArrowBack className='inline ' size={'1.2rem'}  color='white' /><span className='text-xl relative bottom-1 left-3 text-white'>Back</span></div></button>
            )
          }
          <h1 style={{ fontSize: '2rem' }} className={`text-white ${jost.className} mt-5`}>
          ...
          <div className="mt-5 absolute left-24 md:left-40 lg:left-auto flex flex-col bottom-3 justify-center w-[50vw] mx-auto align-middle content-center ">
            ...
            <div className="bg-gray-300 h-4 w-full rounded-md">
              <div className="bg-cyan-500 h-4 rounded-md" style={{ width: `${completionPercentage}%` }}></div>
            </div>
          </div>
        </div>
      </div>
